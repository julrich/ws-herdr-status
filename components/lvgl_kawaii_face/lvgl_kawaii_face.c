...
#include <stdlib.h>
#include <string.h>   /* memset: the ESP headers used to pull this in */
#include <math.h>
...
esp_err_t face_animation_init(face_config_t *config)
{
    ...
    /* The face is something to look at, not to touch: its container covers the whole
     * panel, so left clickable it would take every tap meant for the UI under it. */
    lv_obj_clear_flag(face_state.face_container,
    ...
    face_state.left_eye_buf = FACE_MALLOC_CANVAS(eye_buf_size * sizeof(lv_color32_t));
    face_state.right_eye_buf = FACE_MALLOC_CANVAS(eye_buf_size * sizeof(lv_color32_t));
    face_state.mouth_buf = FACE_MALLOC_CANVAS(mouth_buf_size * sizeof(lv_color32_t));
    ...
    lv_obj_set_style_bg_opa(face_state.left_eye_canvas, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(face_state.left_eye_canvas, 0, 0);
    lv_obj_set_style_pad_all(face_state.left_eye_canvas, 0, 0);
    lv_obj_clear_flag(face_state.left_eye_canvas, LV_OBJ_FLAG_CLICKABLE);
    ...
    lv_obj_set_style_bg_opa(face_state.right_eye_canvas, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(face_state.right_eye_canvas, 0, 0);
    lv_obj_set_style_pad_all(face_state.right_eye_canvas, 0, 0);
    lv_obj_clear_flag(face_state.right_eye_canvas, LV_OBJ_FLAG_CLICKABLE);
    ...
    lv_obj_set_style_bg_opa(face_state.mouth_canvas, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(face_state.mouth_canvas, 0, 0);
    lv_obj_set_style_pad_all(face_state.mouth_canvas, 0, 0);
    lv_obj_clear_flag(face_state.mouth_canvas, LV_OBJ_FLAG_CLICKABLE);
...
static void draw_eye(lv_obj_t *canvas, uint8_t openness, bool is_left)
{
    ...
    lv_canvas_fill_bg(canvas, lv_color_black(), LV_OPA_TRANSP);   /* start clear, not white */
